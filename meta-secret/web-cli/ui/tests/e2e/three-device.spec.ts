...
test('Three Device (Web + Android + iOS) - vault create, join, approve, secret sync, recovery', async ({ browser }) => {
  test.setTimeout(240000); // 4 minutes - Android + iOS tests + web polling
  ...
  try {
    iosProcessPromise = new Promise((resolve) => {
      ...
      const xcodebuildCmd = `xcodebuild test -scheme iosApp -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.5'`;

      console.log(`⏳ Spawning iOS test via xcodebuild`);
  ...
  // ===== PHASE 12: Click Recover =====
  console.log('\n✅ Phase 12: Web - Click Recover button');
  ...
  const iosResult = await iosProcessPromise;
  if (!iosResult.success) {
    console.error(`iOS test failed: ${iosResult.error}`);
  }

  console.log('\n🎉 Three-device setup complete! Awaiting recovery request alerts...\n');
