...
const currentDeviceId = computed(() => {
  try { return (appState.currState as any).device_id().wasm_id_str(); }
  catch { return ''; }
});
...
const isPending = computed(() => props.membership.is_outsider() && props.membership.as_outsider().status === UserDataOutsiderStatus.Pending);
const isDeclined = computed(() => props.membership.is_outsider() && props.membership.as_outsider().status === UserDataOutsiderStatus.Declined);
...
<template>
  ...
  >
    ...
    <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-muted text-primary">
      <svg v-if="deviceDisplay.category === DeviceUiCategory.Iphone" width="18" height="22" viewBox="0 0 60 59" fill="none">
        <rect x="14" y="5" width="32" height="50" rx="6" fill="currentColor" opacity="0.15" stroke="currentColor" stroke-width="2"/>
        <rect x="24" y="-1" width="13" height="11" rx="2" fill="currentColor" opacity="0.5"/>
      </svg>
      <svg v-else-if="deviceDisplay.category === DeviceUiCategory.Android" width="18" height="22" viewBox="0 0 60 59" fill="none">
        <rect x="14" y="5" width="32" height="50" rx="6" fill="currentColor" opacity="0.15" stroke="currentColor" stroke-width="2"/>
        <circle cx="30" cy="10" r="2.2" fill="currentColor"/>
      </svg>
      <svg v-else-if="deviceDisplay.category === DeviceUiCategory.Desktop" width="28" height="18" viewBox="0 0 54 36" fill="none">
        <rect x="5.5" y="1.5" width="44" height="33" rx="3.3" fill="currentColor" opacity="0.15" stroke="currentColor" stroke-width="2"/>
        <path d="M0 31.2H54V33.4C54 34.6 53 35.6 51.8 35.6H4.4C2 35.6 0 33.7 0 31.2Z" fill="currentColor" opacity="0.5"/>
      </svg>
      <svg v-else-if="deviceDisplay.category === DeviceUiCategory.Web" width="20" height="20" viewBox="0 0 54 54" fill="none">
      ...
      <svg v-else-if="deviceDisplay.category === DeviceUiCategory.Cli" width="26" height="18" viewBox="0 0 58 44" fill="none">
        <rect x="1" y="1" width="56" height="42" rx="3" fill="currentColor" opacity="0.15" stroke="currentColor" stroke-width="2"/>
        <polyline points="8,22 14,28 8,34" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
  ...
  <!-- Join confirm dialog -->
  <Dialog :open="isJoinConfirmOpen" @update:open="(v) => { if (!v && !isSubmitting) isJoinConfirmOpen = false; }">
