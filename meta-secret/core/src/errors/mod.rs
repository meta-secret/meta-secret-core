...
pub enum CoreError {
    ...
    #[error(
        "The key manager: {key_manager_pk:?} is not a component of the secure communication channel: {channel:?}"
    )]
...
mod tests {
    ...
    fn test_core_error_from_recovery_error() {
        ...
        match core_error {
            CoreError::RecoveryError { source } => match source {
    ...
    fn test_split_error_from_io_error() {
        ...
        match split_error {
            ...
            SplitError::UserShareJsonSerializationError { .. } => {
                panic!(
                    "Expected SecretsDirectoryError variant, got UserShareJsonSerializationError"
                );
            }
        }
    }
    ...
    fn test_core_error_from_shares_loader_error() {
        ...
        match core_error {
            CoreError::SharesLoaderError { source } => match source {
