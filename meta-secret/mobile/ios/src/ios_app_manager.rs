...
use crate::ios_repo::IosRepo;

pub struct IosApplicationManager {
    app_manager: ApplicationManager<IosRepo, HttpSyncProtocol>,
}
...
impl IosApplicationManager {
    pub async fn init_ios(master_key: TransportSk) -> anyhow::Result<IosApplicationManager> {
        ...
        let client_repo = Arc::new(match IosRepo::default().await {
            Ok(repo) => {repo}
            Err(err) => {
                bail!("Could not initialize ios repo: {}", err);
            }
        });
        let app_manager =
            ApplicationManager::<IosRepo, HttpSyncProtocol>::init(client_repo, master_key)
                .await?;
