...
# Проверка наличия cbindgen
if ! command -v cbindgen &> /dev/null; then
    echo "Installing cbindgen..."
    cargo install cbindgen
fi

# Установка целевых архитектур
echo "Adding Android targets..."
rustup target add aarch64-linux-android armv7-linux-androideabi i686-linux-android x86_64-linux-android

# Проверка и поиск NDK
...
# Проверяем, существуют ли нужные инструменты clang
...
# Экспорт переменных для rustls вместо native-tls
...
# Переходим в директорию проекта Android
cd android
...
# Строим для каждой архитектуры и создаем .so
