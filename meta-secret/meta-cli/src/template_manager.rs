...
impl TemplateManager {
    ...
    fn new() -> Result<Self> {
        ...
        tera.add_raw_template("recovery_claims.yaml", include_str!("templates/recovery_claims.yaml.tera"))?;
        tera.add_raw_template("secrets.yaml", include_str!("templates/secrets.yaml.tera"))?;
        tera.add_raw_template("vault_events.yaml", include_str!("templates/vault_events.yaml.tera"))?;
    ...
    pub fn render(&self, base_template_name: &str, context: &Context, format: CliOutputFormat) -> Result<String> {
