...
impl AcceptRecoveryRequestCommand {
    ...
    pub async fn execute(&self) -> Result<()> {
        info!("Accepting recovery request with claim ID: {}", self.claim_id);
        ...
        match app_state {
            ...
            ApplicationState::Vault(vault_info) => match vault_info {
                ...
                VaultFullInfo::Member(member_info) => {
                    ...
                    let found_recovery_request = member_info
                        .ss_claims
                        .claims
                        .iter()
                        .find(|(id, claim)| {
                    ...
                    match found_recovery_request {
                        Some((_, claim)) => {
                            info!("Found recovery request for password: {}", claim.dist_claim_id.pass_id.name);
