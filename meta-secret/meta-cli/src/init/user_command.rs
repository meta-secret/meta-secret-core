...
impl InitUserCommand {
    ...
    pub async fn execute_with_context<Repo: meta_secret_core::node::db::repo::generic_db::KvLogEventRepo>(
        &self, 
...
#[cfg(test)]
mod tests {
    use meta_secret_core::node::db::descriptors::creds::{DeviceCredsDescriptor, UserCredsDescriptor};
    use super::*;
    ...
    async fn test_init_user_command() -> Result<()> {
        ...
        // First, execute the device command (as shown in Taskfile.yml: device init before user init)
        let init_device_cmd = InitDeviceCommand::new(
            "in_memory_db".to_string(),
        ...
        assert!(device_result.is_ok(), "Device initialization should succeed but failed with: {:?}", device_result);
        ...
        assert!(maybe_device_creds.is_some(), "Device credentials should exist after device initialization");
        
        // Create the user command instance
        let init_user_cmd = InitUserCommand::new(
            "in_memory_db".to_string(),
            vault_name.clone(),
        );
        ...
        assert!(result.is_ok(), "User initialization command should succeed but failed with: {:?}", result);
        
        // Verify that user credentials were created correctly
        let maybe_user_creds = db_context.p_obj.find_tail_event(UserCredsDescriptor).await?;
        assert!(maybe_user_creds.is_some(), "User credentials should exist after user initialization");
        
        // Verify the vault name is correct
        let user_creds = maybe_user_creds.unwrap().value();
        assert_eq!(
            user_creds.vault_name.0, 
            vault_name.0,
        ...
        assert!(second_result.is_err(), "Second execution should fail but succeeded");
