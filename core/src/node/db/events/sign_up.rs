...
use crate::node::db::models::{
    GenericKvLogEvent, KvKey, KvLogEvent, LogEventKeyBasedRecord, ObjectCreator, ObjectDescriptor,
    ObjectType, PublicKeyRecord, VaultObject
};
...
impl SignUpAction {
    pub fn accept(
        ...
        match sign_up_request.key.obj_id.clone() {
            ObjectId::Unit { .. } => {
                match sign_up_request.key.object_type {
                    ObjectType::VaultObj => {
