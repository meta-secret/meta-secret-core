...
pub fn generate_vaults_genesis_key() -> KvKey {
    let id = KvKeyId::genesis(store_names::VAULTS_IDX);
...
pub fn vaults_index_event(prev_id: &KvKeyId, vault_id: &str) -> KvLogEvent {
    KvLogEvent {
...
pub mod test {
    ...
    fn vaults_index_test() -> Result<(), LogCommandError> {
        ...
        let vaults_index_event = vaults_index_event(&genesis_event.key.id, vault_id.base64_text.as_str());
