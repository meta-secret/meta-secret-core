...
#[async_trait(? Send)]
impl<T> DataSync for T
where
    T: PersistentObjectRepo + MetaServerContext,
{
    async fn sync_data(&self, request: SyncRequest) -> Vec<KvLogEvent> {
...
#[async_trait(? Send)]
impl<T> DataTransport for T
where
    T:  KvLogEventRepo + SignUpAction + GlobalIndexAction + MetaServerContext,
{
...
#[async_trait(? Send)]
pub trait MetaServer: KvLogEventRepo + MetaServerContext + SignUpAction + GlobalIndexAction + MetaServerContext {
